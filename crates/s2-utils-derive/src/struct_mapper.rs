...
fn parse_map_from(items: Vec<&NestedMeta>) -> FromStruct {
    if items.is_empty() {
        panic!("#[structmapper(...)] expects at least 1 item");
    }
    ...
    let (from_ty, has_lifetime) = match *ty_item {
        ...
        NestedMeta::Meta(Meta::NameValue(MetaNameValue {
            ...
            if lifetime_count > 1 {
                panic!("#[structmapper(...)] from type has more than 1 lifetime");
            }

            (ty.into_tokens(), lifetime_count > 0)
        }
        _ => {
            panic!("#[structmapper(...)] first item should be a Type or 'From=\"Type\"'");
        }
    };
    ...
    for item in items.iter().skip(1) {
        match **item {
            ...
            NestedMeta::Meta(Meta::NameValue(MetaNameValue {
                ...
                _ => {
                    panic!("#[struct_mapper(...)] unknown item '{}'", ident);
                }
            },
            _ => {}
        }
...
fn parse_field_config(item: &NestedMeta) -> FieldConfig {
    ...
    ref rest => {
      panic!("#[struct_mapper(T, fields(...)] unexpected field config item '{}'. expecting 'ident' or 'ident = \"expr\"'", quote! { #rest })
    },
  }
}
