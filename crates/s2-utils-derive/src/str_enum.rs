  ...
  match variant.fields {
    ...
    Fields::Unnamed(ref fields) if is_unknown => {
      match fields.unnamed.iter().next() {
        ...
        None => unreachable!(),
        _ => panic!("#[derive(StrEnum)] Unknown variant should have String as the only field"),
      }
      VariantConfig {
        name: variant_name,
        name_span: variant.ident.span,
    ...
    _ => panic!("#[derive(StrEnum)] Expects a Unit variant or Tuple variant with name 'Unknown'"),
  }
}
